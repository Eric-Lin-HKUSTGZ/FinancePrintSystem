...
BASE_STATIC_DIR = "./test_file2/static"
...
# 创建目录
os.makedirs(os.path.join(BASE_STATIC_DIR, "merged"), exist_ok=True)
...
async def set_guid(request: GuidRequest):
    ...
    try:
        ...
        if guid_data and not guid_data.get("processing", True) and guid_data.get("files"):
            # 有缓存数据且处理完成且有文件
            active_users = len(guid_data.get("users", set()))
            return JSONResponse(
                content={
                    "status": "success",
                    "message": f"GUID {request.guid} 数据已缓存，立即可用（共{active_users}个用户访问）",
                    "guid": request.guid,
            ...
            # 没有缓存，或缓存为空，或还在处理中，触发下载
            print(f"[DEBUG] set_guid: scheduling process_files_async for guid={request.guid}")
...
async def process_files_async(guid: str):
    ...
    with state_manager.state_lock:
        ...
        if not guid_data["processing"]:
            print(f"[缓存] GUID {guid[:8]}... 数据已存在，跳过处理")
            return
        guid_data["processing"] = True
...
def word_to_pdf(word_path: str) -> str:
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # 使用临时目录，不依赖特定会话
            final_dir = os.path.join(TEMP_DIR, "word_to_pdf_temp")
...
def clean_session_directory(session_id: str):
    """安全地清理单个会话目录"""
    try:
        session_dir = Path("./test_file2/sessions") / session_id
...
def create_interface():
    ...
    with gr.Blocks(title="PBMS文件合并工具", css=custom_css, js=js_func) as demo:
            ...
            try:
                ...
                for index, guid in enumerate(selected_guids, 1):
                    ...
                    if not file:
                        ...
                        continue
                    task = asyncio.create_task(process_file_for_merge(file))
        ...
        async def process_file_for_merge(file: Dict) -> Optional[str]:
            """处理单个文件用于合并"""
            file_path = file["path"]
            ...
            try:
                if file_ext in ('.png', '.jpg', '.jpeg'):
                    return await asyncio.get_event_loop().run_in_executor(
                        thread_pool, image_to_pdf, file_path
                    )
                elif file_ext in ('.doc', '.docx'):
                    return await asyncio.get_event_loop().run_in_executor(
                        thread_pool, word_to_pdf, file_path
                    )
                elif file_ext == '.pdf':
                    return file_path
                else:
